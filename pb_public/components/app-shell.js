customElements.define("ns-app-shell", class extends HTMLElement {
  connectedCallback() {
    ...
    const eventsView = this.querySelector("ns-events-view");
    ...
    window.addEventListener("nav-change", (e) => {
      ...
      if (index === 2) { show(events); eventsView?.load?.(); }
      if (index === 3) { show(groups); groupsView?.load?.(); }
    ...
    window.addEventListener("force-tab", (e) => {
      ...
      if (e.detail?.tab === "dashboard") show(dashboard);
